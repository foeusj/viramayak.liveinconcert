...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (audi3dScene && audi3dCard) {
    ...
    // Touch Dragging Support for Mobile Devices & Smartphones
    audi3dScene.addEventListener('touchstart', (e) => {
    ...
    audi3dScene.addEventListener('touchmove', (e) => {
      if (isDragging && e.touches && e.touches.length === 1) {
        const deltaX = e.touches[0].clientX - startX;
        const deltaY = e.touches[0].clientY - startY;
        currentRotateY += deltaX * 0.22;
        currentRotateX -= deltaY * 0.22;
        currentRotateX = Math.max(-25, Math.min(25, currentRotateX));
        currentRotateY = Math.max(-35, Math.min(35, currentRotateY));
        set3dTransform(currentRotateX, currentRotateY, 1.12);
        startX = e.touches[0].clientX;
    ...
    audi3dScene.addEventListener('touchend', () => {
      isDragging = false;
    });
  ...
  // ------------------------------------------------------------------------
  // 9. Official Trailer Lightbox Modal Logic (Native HTML5 Video + Embed Fallback)
  // ------------------------------------------------------------------------
    ...
    if (trailerModal) {
      ...
      if (trailerVideo) {
        ...
        if (playPromise !== undefined) {
          playPromise.catch(() => {
            // Fallback to iframe if direct play is restricted
            if (trailerIframe) {
